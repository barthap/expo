...
class SQLiteDatabase {
    ...
    exec(queries, readOnly, callback) {
        ...
        SweetSQLite.exec(this._name, queries.map(_serializeQuery), readOnly).then(nativeResultSets => {
...
export class SweetSQLiteDatabase {
    ...
    async executeSql(query) {
        const [result] = await SweetSQLite.exec(this.name, [_serializeQuery({ sql: query, args: [] })], false);
...
export class ExpoSQLiteDatabase {
    ...
    async close() { }
    async singleTransaction(transactionBlock) {
        try {
...
(function (TransactionState) {
    ...
    TransactionState[TransactionState["COMITTED"] = 2] = "COMITTED";
    TransactionState[TransactionState["ROLLED_BACK"] = 3] = "ROLLED_BACK";
