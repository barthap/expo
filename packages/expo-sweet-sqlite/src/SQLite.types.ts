...
/**
 * From WebSQL specs {@link https://www.w3.org/TR/webdatabase/#errors-and-exceptions}
...
export class SQLiteError extends CodedError {}
...
export type QueryResult = ResultSet | ResultSetError;
export type SelectResult = Pick<ResultSet, 'rows'> | ResultSetError;
export type InsertResult = Pick<ResultSet, 'insertId' | 'rowsAffected'> | ResultSetError;
export type UpdateResult = Pick<ResultSet, 'rowsAffected'> | ResultSetError;

/**
 * An interface for basic SQL Operations and convenience methods
 */
export interface SQLInterface {
  // convenience methods
  select(sql: string, args?: any[]): Promise<SelectResult>;
  insert(sql: string, args?: any[]): Promise<InsertResult>;
  updateDelete(sql: string, args?: any[]): Promise<UpdateResult>;

  executeSql(query: Query | string): Promise<QueryResult>;
}

/**
 * An extension to {@link SQLInterface} with transaction-specific operations
 */
export interface ExpoSQLTransaction extends SQLInterface {
  commit(): Promise<void>;
  rollback(): Promise<void>;
}

export type ExpoSQLiteTransactionBlock = (tx: ExpoSQLTransaction) => Promise<void>;
